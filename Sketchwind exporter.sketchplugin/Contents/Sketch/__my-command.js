...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/e6a465b06a67b47dcd710a9b51417503.html";
...
*/
// get infos from each symbols with a name starting with 'colors/' or 'spacings/':

symbols.forEach(function (symbol) {
  /* split symbol name:            example: 'colors/gray/900'
     item[0] is the 'category'              'colors'
     item[1] is the item itself             'gray'
     item[2] is the key for variation       '900'
  */
  var item = symbol.name.replace(/\/\s*$/, '').split('/');

  if (item[0] === 'colors') {
    var color = symbol.layers[0].style.fills[0].color;

    if (symbol.layers[0]) {
      addColor(item, color);
    }
  } else if (item[0] === 'spacings') {
...
function addColor(item, colorValue) {
  ...
  newColor.value = colorValue; // create color object if color doesn't exist

  if (!(newColor.name in theme.colors)) theme.colors[newColor.name] = {}; // create color variation if it exists
...
function addSpacing(item, spacingValue) {// TODO some stuff here to export the spacings
